...
| **Brain (TEE-attested model)** | *not yet pinned live* — the live brain reasons inside a **0G Compute TEE** (the deterministic stub backs the offline build); a plan is labelled `tee` ONLY when the enclave attestation verifies (`web/src/proofs.ts`) | — (0G Compute TEE attestation is the §9 **Depth** bracket-delta; pinned evidence flips the stamp green) | ⏳ honestly deferred, never pre-claimed |
