...
def rigor_card(cap=None):
    ...
    rows=[("verify the code","clean-room, MIT-licensed, reproducible - read every line yourself",CYAN),
    ...
    d.text((W//2,y+205),"hundreds of tests - Rust - Solidity - TypeScript - clean-room - MIT",font=f_ui(28),fill=DIM,anchor="mm")
    watermark(d)
    if cap: lower_third(d,cap)
    return img
...
if __name__=="__main__":
    ...
    rigor_card(cap="cap + gas-floor + net-worth-floor  -  clean-room  -  MIT").save(os.path.join(out,"sample_rigor.png"))
    ...
    title_card("The verification trio","three independent axes - each provable on its own",
        chips=[("VERIFY CODE","clean-room, open, MIT",CYAN),("BOUND SPEND","the on-chain mandate",GOLD),("PROVE SETTLE","the independent verifier",GREEN)],
