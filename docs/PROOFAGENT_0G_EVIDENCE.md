...
|---|---|---|
| **0G Compute TEE-attested brain** (`agent/src/zerog/computeBrain.ts`) | **Depth** | **LIVE** (original clean-room impl on the OFFICIAL `@0gfoundation/0g-compute-ts-sdk` broker): `planZeroGCompute` mints a `"tee"` plan ONLY when `processResponse` verifies the per-response enclave signature (never the model's words). A real attestation verified this session — provider `0xa48f…7836`, model `qwen/qwen2.5-omni-7b`, `processResponse === true` — pinned in `web/src/spine.ts` (`BRAIN_ONCHAIN`), so the brain stamp renders **LIVE**. A TEE attestation is a one-time enclave signature, reproducible via the official SDK (not a re-fetchable hash); absent/un-attested it drops to PENDING — see §1h |
| **0G Storage verdict-bundle publish** (`zerog/storage.ts`) | **Wow** | **LIVE** (original clean-room seam on the OFFICIAL `@0gfoundation/0g-storage-ts-sdk`): `serializeVerdictBundle` → deterministic canonical bytes; `publishVerdictBundle` → the on-0G Merkle `rootHash` (never fabricated — a publish failure degrades LOUD); `liveStorageProvider` did a real `Indexer.upload` of a verdict bundle → **rootHash `0x6b51c075…2fe3f6b`** / txHash `0xb7e7f04f…48f6582`, pinned in `web/src/spine.ts`, so the Storage stamp renders **LIVE** (re-fetchable on storagescan-galileo) |
| **0G ERC-7857 Agentic ID** (`contracts/src/AgentIdentity.sol`) | **Depth (Agentic ID)** | **LIVE** (clean-room ERC-7857 iNFT, zero imports, 14 Foundry tests; adversarially reviewed — 2 MEDIUMs fixed): deployed + token #1 minted on Galileo (`AgentIdentity 0x3A91…c4B1`, deploy `0x5dd5a812…`, mint `0x4ed8e1a2…`). Binds agent + `MandateRegistryV4` + the 0G Compute TEE oracle + the attested model + the 0G Storage mind-handle; `canSpend(1,NATIVE,1e18)` staticcalls the live mandate on-chain → **`OVER_TX_CAP`** (the iNFT enforces the rails). Pinned in `web/src/spine.ts` (`AGENT_ID_ONCHAIN`) → the 5th 0G pillar renders **LIVE** |
