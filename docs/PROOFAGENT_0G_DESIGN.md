...
> **`storage.ts`** (publishing the verifier's verdict bundle to 0G Storage, the **Wow** bracket) is
> **LIVE** — an original clean-room seam on the OFFICIAL `@0gfoundation/0g-storage-ts-sdk`
> (`serializeVerdictBundle` → deterministic canonical bytes; `publishVerdictBundle` → the on-0G Merkle
> `rootHash`; `liveStorageProvider` did a real `Indexer.upload`). A real verdict bundle is published and its
> `rootHash` (`0x6b51c0…3f6b`) is pinned in `web/src/spine.ts`, so the Storage stamp renders **LIVE**. The
...
- **`zerog/`** — the 0G-native depth: TEE-verified inference + auditable storage, both on the OFFICIAL 0G SDKs and both **LIVE**. The TEE-inference **brain** (`computeBrain.ts`, on `@0gfoundation/0g-compute-ts-sdk`) is the §9 *Depth* bracket: `planZeroGCompute` mints a `"tee"` plan ONLY when `processResponse` verifies the per-response enclave signature (never the model's words). A real attestation verified this session (provider `0xa48f…7836`, model `qwen/qwen2.5-omni-7b`, `processResponse === true`), pinned in `web/src/spine.ts` (`BRAIN_ONCHAIN`), so the brain renders **LIVE** — a one-time enclave signature, so reproducible via the SDK (not a re-fetchable hash); absent/un-attested it drops back to PENDING. (`compute.ts` is the original `AttestationProvider` / `attestInference` verifier seam.) `storage.ts` (publishing the verdict bundle to 0G Storage, the §9 *Wow* leg, on `@0gfoundation/0g-storage-ts-sdk`) did a real `Indexer.upload` → `rootHash` `0x6b51c0…3f6b` pinned, so the Storage stamp is **LIVE**. **[Bracket-delta now realized]**: the attestation + storage-publish are LIVE with real pinned, re-checkable evidence.
