...
import {DeploySettlementOracle} from "../script/DeploySettlementOracle.s.sol";

/// @title DeploySettlementOracleTest -- dependency-free tests for the {DeploySettlementOracle} script's
///        target-chain guard.
/// @notice Proves the settlement-oracle deploy script is BUILT and SOUND -- specifically its
...
contract DeploySettlementOracleTest {
    ...
    function test_ChainGuard_RejectsForeignChain() public view {
        (bool ok, bytes memory data) = address(script)
            .staticcall(abi.encodeWithSelector(DeploySettlementOracle.requireOgChain.selector, uint256(31337)));
    ...
    function test_ChainGuard_RejectsZeroChain() public view {
        (bool ok,) =
            address(script).staticcall(abi.encodeWithSelector(DeploySettlementOracle.requireOgChain.selector, uint256(0)));
